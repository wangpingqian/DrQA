...
# Defaults
DATA_DIR = os.path.join(DRQA_DATA, 'sogouqa')
...
def add_train_args(parser):
    ...
    files.add_argument('--train-file', type=str,
                       default='train_factoid_1-processed-ltp.txt',
                       #default='test_factoid_1-processed-ltp.txt',
                       help='Preprocessed train file')
    files.add_argument('--dev-file', type=str,
                       default='dev_factoid_1-processed-ltp.txt',
                       #default='SQuAD-v1.1-dev-processed-corenlp.txt',
                       help='Preprocessed dev file')
    #files.add_argument('--dev-json', type=str, default='dev_factoid_1.json',
...
def set_defaults(args):
    ...
    if args.embedding_file:
        with open(args.embedding_file) as f:
            dim = int(f.readline().strip().split(' ')[1])    # for word2vec embeddings
            print("embedding size: {}".format(dim))
            #dim = len(f.readline().strip().split(' ')) - 1  # for glove embeddings
...
def init_from_scratch(args, train_exs, dev_exs):
    ...
    if args.embedding_file:
        model.load_embeddings(word_dict.tokens(), args.embedding_file, word2vec=True)
...
def main(args):
    ...
    if args.official_eval:
        #####################################################
        dev_texts = {ex['id']: ex['normal_context'] for ex in dev_exs}
        dev_offsets = {ex['id']: ex['offsets'] for ex in dev_exs}
        dev_answers = {ex['id']: ex['text_answer'] for ex in dev_exs}
    ...
    if args.checkpoint and os.path.isfile(args.model_file + '.checkpoint'):
        ...
        if args.pretrained:
            ...
            if args.expand_dictionary:
                ...
                if args.embedding_file:
                    model.load_embeddings(added, args.embedding_file, word2vec=True)
